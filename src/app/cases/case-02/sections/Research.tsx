...
// 02 Исследование — на десктопе (≥1440) абсолют 1:1 из Figma (node
// 2009:12143). Ниже 1440 — поток: заголовок + вводный текст по левому краю,
// документ-карточки (единый экспорт 1008×645) на всю ширину сетки,
// доодл-«лупа» только на десктопе.
//
// Отступ текст→карточки (Figma): 375 — 32, 834/1280 — 64, ≥1440 — 166.
// На ≥1440 карточки следуют за текстом (высота текста плавает), а не
// привязаны к фикс-координате — иначе при другом объёме текста зазор плывёт.
//
    ...
    <section className="w-full bg-[#fafafa] xl:relative xl:mx-auto xl:min-h-[1065px] xl:w-[1440px] xl:overflow-clip">
      ...
      <div className="flex flex-col gap-[12px] px-[var(--grid-margin)] py-[64px] sm:gap-[24px] sm:py-[72px] xl:contents">
        ...
        {/* Текст + карточки. Отступ текст→карточки: 375 — 32, 834/1280 — 64,
            ≥1440 — 166. На ≥1440 карточки следуют за текстом (высота текста
            плавает), а не привязаны к фикс-координате. */}
        <div className="flex flex-col gap-[32px] sm:gap-[64px] xl:gap-0 xl:pb-[100px] xl:pl-[46px] xl:pt-[181px]">
          ...
          {!sm ? (
            <button
              ...
              onClick={() => setOpen((o) => !o)}
              aria-expanded={open}
              aria-label={open ? t.collapseCardsLabel : t.expandCardsLabel}
