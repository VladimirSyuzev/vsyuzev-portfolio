...
// Порядок скетч-точек из get_design_context — язык-независимый (текст —
// из словаря C1[lang].guideBullets, 7 пунктов).
const GUIDE_DOTS = [2, 3, 2, 1, 2, 3, 4];

// 4 шага (Figma frame 2009:11514, x46 / y561): диаграмма 328×328 + подпись
// (текст — из словаря C1[lang].guideSteps).
const STEP_IMAGES = [
  `${RA}/role-diagram-1.svg`,
  `${RA}/role-diagram-2.svg`,
  `${RA}/role-diagram-3.svg`,
  `${RA}/role-diagram-4.svg`,
];
const STEP_NUMBERS = ["001", "002", "003", "004"];

function GuideFlow() {
  const lang = useLang();
  const t = C1[lang];
  const guideBullets = t.guideBullets.map((text, i) => ({ text, dot: GUIDE_DOTS[i] }));
  const cols = [guideBullets.slice(0, 4), guideBullets.slice(4)];
  return (
      ...
      <div className="flex flex-col gap-[32px] sm:gap-[64px] lg:gap-[32px]">
        ...
        <div className="flex flex-col gap-[32px]">
          ...
          <div className="flex flex-col gap-[32px] sm:grid sm:grid-cols-2 sm:gap-[12px]">
            {t.guideSteps.map((text, i) => (
              <div key={i} className="flex w-[328px] max-w-full flex-col gap-[24px] sm:h-[430px]">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img alt="" src={STEP_IMAGES[i]} className="block aspect-square w-[328px] max-w-full" />
                ...
                <div className="flex flex-col gap-[12px]">
                  <div className="flex items-center gap-[16px]">
                    ...
                    <p className="text-[14px] font-medium uppercase tracking-[0.28px] text-[#121212]">{STEP_NUMBERS[i]}</p>
                  </div>
...
export default function TeamGuide() {
  ...
  const lang = useLang();
  const t = C1[lang];
  const bulletsLeft = t.guideBullets.slice(0, 4);
  const bulletsRight = t.guideBullets.slice(4);
      ...
      <div className="flex flex-col gap-[28px] px-[var(--grid-margin)] py-[72px] xl:contents">
        ...
        <p className="text-[14px] leading-[1.2] tracking-[0.28px] text-[#121212] opacity-70 sm:max-w-[636px] xl:absolute xl:left-[46px] xl:top-[181px] xl:w-[635.535px]">
          {lang === "ru" ? (
            ...
            <>
              So the two designers could work in sync and get a predictable result,{" "}
              <br className="hidden xl:inline" />I put together an internal guide to making icons. It combined Yandex&apos;s requirements{" "}
              <br className="hidden xl:inline" />with the experience the team built up during the project.
            </>
          )}
        </p>
        ...
        <div className="grid grid-cols-1 gap-[24px] sm:grid-cols-2 xl:absolute xl:left-[46px] xl:top-[561px] xl:flex xl:items-start xl:gap-[12px]">
          {t.guideSteps.map((text, i) => (
            <div key={i} className="group flex flex-col gap-[16px] xl:w-[328px] xl:gap-[24px]">
              ...
              <div className="flex flex-col gap-[12px] xl:h-[78px]">
                <div className="flex items-center gap-[16px]">
                  ...
                  <p className="text-[14px] font-medium tracking-[0.28px] text-[#121212]">{STEP_NUMBERS[i]}</p>
                </div>
